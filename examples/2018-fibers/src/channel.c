...
// Recompute the required fading, to account for bad wheather
static void fading_recheck(lp_state_type *pointer) {
	channel *ch;
...
// Depending on the time, the number of generated calls per second can change
double recompute_ta(double ref_ta, double time_now) {
...
// Accurate simulation of channels require to compute the call gain
static double generate_cross_path_gain(void) {
	double value;
	double variation;
...
// Accurate simulation of channels require to compute the call gain
static double generate_path_gain(void) {
	double value;
	double variation;
...
// A call is over: free the corresponding channel
void deallocation(unsigned int me, lp_state_type *pointer, int ch, double lvt) {
	channel *c;
...
// A call is started: find whether we have a free channel to use for it
